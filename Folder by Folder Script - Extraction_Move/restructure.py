...
CUSTOMER_FOLDER = "Barbasol"  # <-- Change this to the customer folder name
...
LOG_NAME = CUSTOMER_FOLDER.replace(" ", "_") + "_movelog_" + MODE + ".csv"
...
def process_curp_folder(curp_path, sn_map, log, errors):
    ...
    if target_sn:
        ...
        if DRY_RUN:
            ...
            try:
                dest = os.path.join(CUSTOMER_PATH, target_sn, "2_CURP", curp_folder)
                os.makedirs(os.path.dirname(dest), exist_ok=True)
                shutil.move(curp_path, dest)
                log(curp_folder, "folder", curp_path, new_curp_path, "moved")
            except Exception as e:
...
def process_sr_folder(sr_path, sn_map, log, errors):
    ...
    if target_sn:
        ...
        if DRY_RUN:
            ...
            try:
                dest = os.path.join(CUSTOMER_PATH, target_sn, "1_SERVICE", sr_folder)
                os.makedirs(os.path.dirname(dest), exist_ok=True)
                shutil.move(sr_path, dest)
                log(sr_folder, "folder", sr_path, new_sr_path, "moved")
            except Exception as e:
...
def process_sn_folder(sn_folder, log, errors):
    ...
    if os.path.isdir(thumb_path):
                ...
                if fname in reference_files:
                    if not DRY_RUN:
                        try:
                            ...
                            errors.append("ERROR deleting " + fpath + ": " + str(e))
                    log(fname, "file", fpath, None, "deleted (duplicate in THUMB DRIVE_TEMPLATE)")
                else:
        ...
        if not DRY_RUN:
            try:
                ...
                errors.append("ERROR deleting THUMB DRIVE_TEMPLATE: " + str(e))
        log("THUMB DRIVE_TEMPLATE", "folder", thumb_path, None, "deleted (processed)")
        ...
        if os.path.isdir(item_path) and is_calibration_folder(item):
            ...
            if not DRY_RUN:
                try:
                    ...
                    errors.append("ERROR deleting " + item + ": " + str(e))
            log(item, "folder", item_path, None, "deleted (split into template subfolders)")
    ...
    if not DRY_RUN:
        try:
            shutil.move(sn_path, new_sn_path)
            print("  Moved to output: " + new_sn_path)
        except Exception as e:
            err = "ERROR moving SN folder " + sn_folder + ": " + str(e)
            print("  " + err)
            errors.append(err)
            log(sn_folder, "folder", sn_path, new_sn_path, "ERROR: " + str(e))
            return

    log(sn_folder, "folder", sn_path, new_sn_path, "moved")
...
def main():
    ...
    if DRY_RUN:
        ...
        print("Files WILL be moved. This cannot be undone.")
    print("=" * 60)
                    ...
                    if total_files == 0:
                        if not DRY_RUN:
                            try:
                                ...
                                pass
                        log(item, "folder", item_path, None, "deleted (empty container folder)")
                    else:
                        log(item, "folder", item_path, None,
                            "unchanged (container has " + str(total_files) + " files remaining)")
                continue
    ...
    if os.path.isdir(CUSTOMER_PATH):
        ...
        if not remaining:
            ...
            log(CUSTOMER_FOLDER, "folder", CUSTOMER_PATH, None,
                "deleted (empty customer folder)")
        else:
            log(CUSTOMER_FOLDER, "folder", CUSTOMER_PATH, None,
                "unchanged (not empty - " + str(len(remaining)) + " item(s) remain)")
    ...
    print("\n" + "=" * 60)
    print(MODE + " COMPLETE - " + CUSTOMER_FOLDER)
